      ...
      *                                                               *
      * 1).  Browse URIMAP                                            *
      * 2).  Issue START for the zExpire task (ZCEX) for each URIMAP. *
      * 3).  Issue WTO for each START command.                        *
      *                                                               *
      * Date       UserID  Description                                *
      * --------- ------- ------------------------------------------- *
      *                                                               *
      * 2013/01/20 J1FRERK Create base code                           *
      * 2013/05/10 zCache R2.0 support.                               *
RF01  * 2013/05/10 Change transaction ID from ZCEX to ZX##.           *
      *                                                               *
      *****************************************************************
...
RF01   01  ZX-TRANID.
RF01       02  FILLER             PIC  X(02) VALUE 'ZX'.
RF01       02  ZX-SUFFIX          PIC  X(02) VALUE SPACES.

       01  ZCEX                   PIC  X(04) VALUE 'ZCEX'.
       01  CSSL                   PIC  X(04) VALUE 'CSSL'.
       01  TD-QUEUE               PIC  X(04) VALUE '@tdq@'.
       01  TD-LENGTH              PIC S9(04) COMP VALUE ZEROES.

       01  TD-RECORD.
RF01  *    02  FILLER             PIC  X(16) VALUE 'zPLT start ZCEX '.
RF01       02  FILLER             PIC  X(13) VALUE 'zPLT start ZX'.
RF01       02  TD-SUFFIX          PIC  X(02) VALUE SPACES.
RF01       02  FILLER             PIC  X(01) VALUE SPACES.
      ...
      *****************************************************************
      * Check URIMAP for ZC* entries                                  *
      *****************************************************************
       2100-CHECK-URIMAP.
           IF  URI-PREFIX(1:2) EQUAL ZC      AND
      ...
      *****************************************************************
      * Issue START command for zExpire (ZCEX)                        *
      * Issue WRITEQ TD QUEUE(TD-QUEUE)                               *
      * Issue WRITEQ TD QUEUE(CSSL)                                   *
      * Issue WTO                                                     *
      *****************************************************************
       2200-START.
RF01       MOVE URI-PREFIX(3:2)      TO ZX-SUFFIX
RF01                                    TD-SUFFIX.

           MOVE LENGTH OF TD-RECORD  TO TD-LENGTH.

RF01       EXEC CICS START TRANSID(ZX-TRANID)
                FROM(URI-TRAN)
                LENGTH(4)
                NOHANDLE
           END-EXEC.
